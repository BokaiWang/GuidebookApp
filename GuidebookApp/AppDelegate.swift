...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func preloadData() {
        ...
        if defaults.bool(forKey: Constants.PRELOAD_DATA) == false {
            ...
            // Get a path to local json file
            let pathString = Bundle.main.path(forResource: "PreloadedData", ofType: "json")
            
            guard pathString != nil else {
                print("Couldn't get a path to the json file")
                return
            }
            // create a url object using path
            let url = URL(fileURLWithPath: pathString!)
                
            do {
                // create a data object using url object
                let data = try Data(contentsOf: url)
                    
                // Start decoding the data object using JSONDecoder object
